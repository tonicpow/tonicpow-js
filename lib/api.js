...
async function acceptUser (t, userId = 0, email = '') {
  return tonicAxios.put(t.config.apiUrl + apiVersion + '/users/status/accept', { id: userId, email: email }, getOptions())
}
...
module.exports = {
  ...
  acceptUser: async function (userId = 0, email = '') {
    return new Promise(async (resolve, reject) => {
      try {
        initCheck(this.loaded)
        await acceptUser(this, userId, email)
