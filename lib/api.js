...
async function getCampaignsByAdvertiserProfile (t, profileId) {
  return tonicAxios.get(t.config.apiUrl + apiVersion + '/advertisers/campaigns/' + profileId, getOptions())
}
...
async function listCampaigns (t, customSessionToken = '') {
  return tonicAxios.get(t.config.apiUrl + apiVersion + '/campaigns/list', getOptions(customSessionToken))
}
...
async function getCampaignsByUrl (t, url) {
  return tonicAxios.get(t.config.apiUrl + apiVersion + '/campaigns/find?target_url=' + url, getOptions())
}
...
async function getLinksByUserID (t, userId = 0, userSessionToken = '') {
  return tonicAxios.get(t.config.apiUrl + apiVersion + '/links/user/' + userId, getOptions(userSessionToken))
}
...
module.exports = {
  ...
  getCampaignsByAdvertiserProfile: async function (profileId) {
    return new Promise(async (resolve, reject) => {
      try {
        initCheck(this.loaded)
        let response = await getCampaignsByAdvertiserProfile(this, profileId)
        resolve(response.data)
      } catch (e) {
        reject(checkError(e))
      }
  ...
  getLinksByUserID: async function (userId = 0, userSessionToken = '') {
    return new Promise(async (resolve, reject) => {
      try {
        initCheck(this.loaded)
        let response = await getLinksByUserID(this, userId, setUserToken(this, userSessionToken))
        resolve(response.data)
      } catch (e) {
        reject(checkError(e))
      }
