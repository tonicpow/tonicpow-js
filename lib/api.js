...
function extractSessionTokenFromHeader (cookieHeader = '', cookieName = '') {
  ...
  let list = {}
  cookieHeader = cookieHeader.replace('Set-Cookie:', '').replace('Cookie:', '')
...
let session = {
  ...
  // Default options for the cookie generation settings
  expirationInDays: 2,
  httpOnly: false,
  secureCookie: false,
  cookieName: defaultCookieName,
  ...
  set userToken (value) {
    ...
    if (value && value.length > 0) {
      if (value.indexOf(this.cookieName + '=') !== -1) {
        value = value.replace(this.cookieName + '=', '')
      } else if (this.cookieName !== defaultCookieName) {
        value = value.replace(defaultCookieName + '=', '')
      }
  ...
  get userCookie () {
    ...
    if (!this._cookieDomain || this._cookieDomain.length === 0) {
      return ''
    }

    // Build the cookie
    let cookie = this.cookieName + '=' + this._userToken + '; Domain=' + this._cookieDomain + '; Path=/;'
    if (this.expirationInDays > 0) {
      let d = new Date()
      d.setTime(d.getTime() + 24 * 60 * 60 * 1000 * this.expirationInDays)
      cookie += ' Expires=' + d.toGMTString() + ';'
      cookie += ' Max-Age=' + 60 * 60 * 24 * this.expirationInDays + ';'
    }
...
function wrapAxios (t) {
  ...
  axios.interceptors.request.use(function (config) {
    ...
    if (typeof config.headers[internalHeaderKey] !== 'undefined') {
      config.jar.setCookie(defaultCookieName + '=' + config.headers[internalHeaderKey] + '; Max-Age=' + defaultMaxAge + '; Path=/; HttpOnly;', config.url, function (err, cookies) {
        if (err) {
      ...
      config.jar.setCookie(defaultCookieName + '=' + t.session.apiToken + '; Max-Age=' + defaultMaxAge + '; Path=/; HttpOnly;', config.url, function (err, cookies) {
        if (err) {
          throw Error(err.message)
        }
      })
    ...
    response.config.jar.getCookies(response.config.url, { allPaths: true }, function (err, cookies) {
      ...
      if (cookies.length > 0) {
        for (let i = 0; i < cookies.length; i++) {
          if (cookies[i].key === defaultCookieName) {
...
module.exports = {
  ...
  init: async function (apiKey, options = {}) {
    // Load the configuration and options
    config.apiKey = apiKey
    if (options && options.hasOwnProperty('environment')) {
      config.environment = options.environment
    }
    if (options && options.hasOwnProperty('domain')) {
      session.cookieDomain = options.domain
    }
    if (options && options.hasOwnProperty('cookieName')) {
      session.cookieName = options.cookieName
    }
    let customSessionToken = ''
    if (options && options.hasOwnProperty('token')) {
