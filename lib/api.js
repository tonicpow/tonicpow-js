...
async function logoutUser(t, userSessionToken) {
  ...
  // Missing token
  if (!userSessionToken || userSessionToken.length < 10) {
...
module.exports = {
    ...
    return new Promise(async(resolve, reject) => {
      try {
        ...
        if (session.apiToken || (typeof customSessionToken === 'string' && customSessionToken.length > 0)) {
          await prolongSession(this, customSessionToken)
        } else {
  ...
  endSession: async function(customSessionToken){
    return new Promise(async(resolve, reject) => {
      try {
        blockBrowser(this.initRequired())
        await endSession(this, customSessionToken)
  ...
  logoutUser: async function(customSessionToken){
    return new Promise(async(resolve, reject) => {
      try {
        blockBrowser(this.initRequired())
        await logoutUser(this, customSessionToken)
