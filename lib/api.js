...
async function convertGoalWithVisitorSession (t, goalId, tncpwSession, additionalData = '') {
...
// convertGoalWithSession will fire a conversion for a given goal, if successful it will make a new Conversion
//
// For more information: https://docs.tonicpow.com/#d724f762-329e-473d-bdc4-aebc19dd9ea8
async function convertGoalWithUserId (t, goalId, userId, additionalData = '') {
  let data = { name: goalName, user_id: userId, additional_data: additionalData }
  return tonicAxios.post(t.config.apiUrl + version + '/goals/convert', data, getOptions())
}
...
module.exports = {
  ...
  convertGoalWithVisitorSession: async function (goalId, tncpwSession, additionalData = '') {
    return new Promise(async (resolve, reject) => {
      try {
        initCheck(this.loaded)
        let response = await convertGoalWithVisitorSession(this, goalId, tncpwSession, additionalData)
        resolve(response.data)
      } catch (e) {
        reject(checkError(e))
      }
  ...
  convertGoalWithUserId: async function (goalId, userId, additionalData = '') {
    return new Promise(async (resolve, reject) => {
      try {
        initCheck(this.loaded)
        let response = await convertGoalWithUserId(this, goalId, userId, additionalData)
        resolve(response.data)
      } catch (e) {
        reject(checkError(e))
      }
