...
const apiCookieName = 'session_token' // From API requests
const defaultMaxAge = 172800 // From API session cookie time (seconds) (48 hours)
const internalHeaderKey = 'x-user-session-token' // Used internally for communication from req=>resp
const version = 'v1' // Current version for requests from the API
...
let session = {
  ...
  set userToken (value) {
    ...
    if (value && value.length > 0) {
      ...
      } else if (this.cookieName !== apiCookieName) {
        value = value.replace(apiCookieName + '=', '')
      }
