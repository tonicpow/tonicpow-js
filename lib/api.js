...
let internalHeaderKey = "x-user-session-token"
...
module.exports = {
  ...
  prolong: async function(userSessionToken){
    return new Promise(async(resolve, reject) => {
      ...
      try {
        let x = await prolongSession(userSessionToken);
        if (userSessionToken && userSessionToken.length > 0) {
  ...
  currentUser: async function(userSessionToken){
    return new Promise(async(resolve, reject) => {
      ...
      try {
        let x = await currentUser(userSessionToken);
        this.userSessionToken = userLastSessionCookie.value
        resolve({success: "current user"})
      } catch(e){
